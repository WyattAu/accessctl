...
impl Default for RoleHierarchy {
    fn default() -> Self {
        Self {
            roles: vec![Role::Viewer, Role::Editor, Role::Admin],
        }
    }
}
...
impl RoleHierarchy {
    ...
    pub fn check_permission(&self, role: &Role, action: &str) -> bool {
        matches!(
            (role, action),
            (Role::Admin, _) | (Role::Editor, "view" | "edit" | "create") | (Role::Viewer, "view")
        )
    }
}
...
impl PolicySet {
    ...
    pub fn inner(&self) -> &cedar_policy::PolicySet {
        &self.inner
    }
}
